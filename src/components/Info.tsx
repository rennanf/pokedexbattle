...
import { setPokemonTabs } from "../app/slices/AppSlice";
  ...
  const createStatsArray = (types: string[] | undefined, statType: string) => {
    ...
    if (Array.isArray(types)) {
      types.forEach((type: string) => {
        // @ts-ignore
        if (pokemonTypes && typeof pokemonTypes[type] === 'object' && Array.isArray(pokemonTypes[type][statType])) {
          // Adiciona as estatísticas ao conjunto
           // @ts-ignore
  ...
  return (
    <>
      <div className="details">
        ...
        <button onClick={() => dispatch(setPokemonTabs(pokemonTabs.evolution))}>
          See next evolution
        </button>
      </div>
      <div className="stats">
